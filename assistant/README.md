...
## Empfehlungen nach dem Hinzufügen

Nach einem bestätigten nativen Storesynk-`cartUpdate` vom Typ `addToCart` erscheint eine Nachricht mit bis zu drei horizontalen Karten: Match, Bestseller und ähnlicher Stil (Farbe/Muster). Bereits enthaltene Produkte und Dubletten werden ausgeschlossen; maximal ein Mäppchen. Gibt es nicht genügend verfügbare passende Artikel, werden weniger Karten angeboten. Geschlossen bleibt die Nachricht ungelesen und zeigt den roten Indikator; auf Mobil rückt der Launcher dann über die untere Produktleiste.
